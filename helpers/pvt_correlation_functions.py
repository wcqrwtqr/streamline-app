...
def gas_pseudoprops(temp, pressure, sg, x_h2s, x_co2):
    ...
    * Pseudo-reduced properties
    ...
    if (
        ...
        e = (120 * (((x_h2s + x_co2) ** 0.9) - ((x_h2s + x_co2) ** 1.6))) + (
            15 * (x_h2s**0.5 - x_h2s**4)
        )
...
def gas_mu(temp, rhogas, sg):
    ...
    if temp > 100 and temp < 340:
        ...
        rhogas_lee = (
            rhogas * 0.0160185
        )  # lbm/ft3 converted to gas density unit of Lee et al (g/cm3)
...
def gas_compressibility(T_pr, P_pr, rho_pr, z, P_pc):
    ...
    do = (
        (
            (a1 + (a2 / T_pr) + (a3 / T_pr**3) + (a4 / T_pr**4) + (a5 / T_pr**5))
            * rho_pr
        )
        + (2 * ((a6 + (a7 / T_pr) + (a8 / T_pr**2))) * rho_pr**2)
        - (5 * a9 * (((a7 / T_pr) + (a8 / T_pr**2))) * rho_pr**4)
        + (1 + (a11 * rho_pr**2) - (a11 * rho_pr**2) ** 2)
        * ((2 * a10 * rho_pr / T_pr**3) * np.exp(-a11 * rho_pr**2))
    )

    c_pr_analytical = (1 / P_pr) - (
        (0.27 / (z**2 * T_pr)) * (do / (1 + ((rho_pr / z) * do)))
    )
...
def oil_pbubble(Rsb, sg2, api, temp2):
    ...
    if (
        ...
        P_bubble = (Rsb / (c1 * sg2 * np.exp((c3 * api) / (temp2 + 459.67)))) ** (
            1 / c2
...
def oil_fvf(P_bubble, api, Rsb, sg2, temp2, pressure2):
    ...
    * At and bubble-point pressure
    ...
    Bo_bubble = (
        1
        + ((0.0005 * Rsb) * ((sg2 / so) ** 0.25))
        + ((0.0004 * (temp2 - 60)) / (so * sg2))
    )  # temp in def F
    ...
    if pressure2 > P_bubble:
        ...
        coil = ((5 * Rsb) + (17.2 * temp2) - (1180 * sg2) + (12.61 * api) - 1433) / (
            1e05 * pressure2
        )
...
def oil_compressibility(pressure2, P_bubble, temp2, api, Rsb, sg2):
    ...
    * Above and at bubble-point pressure
    ...
    if pressure2 < P_bubble:
        ...
        ln_coil = (
            -7.573
            - (1.45 * np.log(pressure2))
            - (0.383 * np.log(P_bubble))
            + (1.402 * np.log(temp2))
            + (0.256 * np.log(api))
            + (0.449 * np.log(Rsb))
        )
